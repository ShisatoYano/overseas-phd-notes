...
## 【2026-09-22 追記】必須条件の変更: 在職継続(External / Part-time)

現職を続けながら学位を取ることも必須条件に加わった。したがって候補の判定軸は**「リモート可」×「在職可」の2軸**になり、片方だけでは足切りされる。

### 用語の整理(調査時に混同しやすい)

- **Part-time PhD**(英・豪): 大学に在籍したまま修業年限を延ばす形式。英国では標準6年。**Distance Learning と組み合わせられるかは大学・学科ごとに異なる**ため、「パートタイムがある」だけでは不十分で、必ず「Distance × Part-time」の組み合わせが提供されているかを確認する
- **External PhD / buitenpromovendus**(蘭): 大学に雇用されない(給与なし)前提で、外部に職を持ったまま学位を目指す正規ルート。TU Delft・Twente・Eindhoven 等。**リモート可否は別問題**で要確認
- **Professional Doctorate / EngD**: 在職前提の設計だが学位が PhD とは異なる。候補に含めるかは方針判断(現時点では PhD を優先)

### ビザに関する重要な含意

日本からのフルリモートであれば**学生ビザを取得しない**ため、「学生ビザは就労を制限する」という在職両立の通常最大の障壁が発生しない。残る制約は (1) 大学の学則がパートタイム/External を認めるか、(2) 実際の時間確保、の2点に絞られる。

### 既存記述への影響

- **Swansea**: EEE の Distance PhD は**フルタイム3年のみで在職不可 → 脱落**。Computer Science は Distance × パートタイム6年を公式提供しており**条件を満たす**(国際生 £11,800/年)。詳細は `labs/swansea.md`
- **UTS**: 調査済み(`labs/uts.md`)。パートタイム最長8年の制度はあるが、**PhD by distance との組み合わせは公式に明記されておらず推定止まり**。一方、UTSの公式文面がビザ論法(下記)を裏付けた
- Split-site / Sandwich 型は現地滞在期間が数か月〜1年あるため、在職継続との相性は悪い。**優先度を下げる**

### 許容年限

6〜7年(英国パートタイムの標準6年)を前提に候補を評価する。

### 【2026-09-22 追記2】ビザ論法の裏付け(UTS調査より)

UTSの公式記述に次の一文がある。

> distance mode is available for international students. However, to obtain a student visa to study in Australia, international students must enrol **full time and on campus**, which means distance study may not be available to international students requiring a visa.
...
### 【2026-09-26 追記4】勤務先との合意書が実質的な前提条件になる

Swansea の CS Admissions Tutor から、在職のまま進める場合は**勤務先との正式な合意書を強く勧める**との回答を得た(`contacts/admin-inquiries.md` 受領記録)。取り決めるべきとされた点は次の3つ。

1. 勤務時間を研究に使える範囲
2. 勤務先の設備を論文作業に使えるか
3. **成果の公開に勤務先がかける制限(最重要)**

これは Swansea に限らない。どの大学でも、在職のまま進めるなら同じ論点が出てくる。自動運転は企業側の機密が絡みやすい分野なので、特に3がボトルネックになりやすい。PhD では論文として成果を公開することが学位の要件になるため、公開できないテーマは研究テーマとして成り立たない。

→ `research-theme/` でテーマを選ぶ際は、**業務と切り離して公開できるか**を判断基準に加える。

#### 勤務先との合意済み事項(2026-09-26 時点)

留学については既に勤務先と相談しており、次の形で合意している。

- **勤務先と大学の共同プロジェクトにはしない**
- **研究テーマは自分で自由に決めてよい**
- **勤務時間をいくらか短縮し、研究時間を確保できるよう支援してもらえる**
- その代わり、**勤務先の設備・コード・開発環境などのリソースは研究に使わない**

つまり「勤務先が支援するプロジェクト」ではなく、**勤務先から独立した個人の研究**として進める。これによる影響は次のとおり。

- 公開制限の論点は、勤務先の資産を持ち込まない分、大幅に小さくなる
- Swansea が示した「必要な設備は勤務先が提供してもよい」という選択肢は使えない。研究は**個人の計算環境と公開データ・OSSで完結させる**必要がある(既存のシミュレーション中心方針と一致)
- 自作のOSS(AutonomousVehicleControlBeginnersGuide)は個人の成果物であり、研究の土台として使える
- 勤務時間を短縮できるため、パートタイム(6年)の研究時間と週1回程度の面談を確保しやすい。出願時に「研究時間の裏付け」として示せる

まだ詰めていない点(必要になった時点で書面化する):

1. ~~研究は勤務時間外に行う前提か~~ → 勤務時間の短縮で研究時間を確保する形で合意済み。短縮の幅と開始時期は未定
2. 業務と近い分野(自動運転)の研究成果を、勤務先の事前確認なしに公開してよいか。職務発明規程や秘密保持義務との関係
   - 見通し(未確認): **勤務先の非公開情報を含まなければ、おそらく可能**
3. 大学が求める場合に、勤務先の同意書(no-objection letter)を出してもらえるか
   - 見通し(未確認): 留学そのものは後押しされているので、**共同研究への同意といった重い内容でなく、単に在籍を認める趣旨の書面なら、おそらく可能**

→ 2・3 はどちらも出願前に確認して書面にする。研究テーマを選ぶ段階で、**勤務先の非公開情報に依存しないテーマか**を必ず確認する

また、ATAS(英国の安全保障審査)は学生ビザの対象にならない distance 生には不要だろう、との見解も得た(本人は法務の専門外と断っている)。ビザ論法が英国の審査制度にも及ぶ傍証になる。
